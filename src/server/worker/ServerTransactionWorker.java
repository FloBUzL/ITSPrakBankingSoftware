...
public class ServerTransactionWorker extends ServerWorker {
	...
	private void checkAuthentication() throws Exception {
		...
		if (this.connectionData.isAuthenticated()) {
			this.connectionData.getConnection()
					.write(new Message().addData("task", "transaction").addData("message", "pass_authentication"));
			return;
		}

		String nonce = new RandomString(32).toString();
		Message askForAuthentication = new Message().addData("task", "transaction")
				.addData("message", "request_authentication").addData("nonce", nonce);
		Message finishRegistration = new Message().addData("task", "transaction").addData("message",
				"finish_registration");
		Message authCodeAnswer = new Message().addData("task", "transaction");
		boolean loop = true;
		...
		while (loop) {
			...
			case "authenticate_device":
				...
				Message authReply = new Message().addData("task", "transaction");

				// check if user authenticated successfully
				if (this.connectionData.getDatabase().checkAuthCR(this.connectionData.getUserName(), deviceCode, nonce,
						clientCR)) {
					loop = false;
			...
			case "register_device":
				...
				// checks auth code
				if (this.connectionData.getDatabase().checkAuthCR(this.connectionData.getUserName(),
						clientCode + serverCodeFirstPart + serverCodeSecondPart, nonce, cr)) {
					this.connectionData.authenticate();
					...
					this.connectionData.getDatabase().deleteDevice(this.connectionData.getUserName(),
							clientCode + serverCodeFirstPart + serverCodeSecondPart);
					this.connectionData.getAuthErrors().increase(this.connectionData.getConnection().getIP(),
	...
	private void handleTransaction() throws Exception {
		...
		String receiver;
		String amount;
		Message transactionResponse = new Message().addData("task", "transaction");
		...
		if (this.connectionData.getDatabase().doTransaction(this.connectionData.getUserName(), receiver,
